...
!
! key fixes:
!  - apply the body-force kick ONLY once per GLOBAL timestep (gate on `step`)
!    so we do not double/quadruple count when TRISTAN subcycles internally.
!  - use dt_loc = 1.0 in code time units for this once-per-step kick.
!  - use a simple, transparent explicit update (Euler) for correctness.
!    (once the epicycle test passes, you can upgrade to a higher-order/split scheme.)
!--------------------------------------------------------------------
subroutine userDriveParticles(step)
  ...
  real :: v2, scale
  real :: dt_loc
  integer, save :: last_step_seen = -999999999

  if (.not. use_rotation) return

  !------------------------------------------------------------
  ! gate: ensure we apply this forcing ONCE per global timestep.
  ! TRISTAN may call userDriveParticles multiple times per step
  ! (subcycling / internal stages). If `step` repeats, skip.
  !------------------------------------------------------------
  if (present(step)) then
    if (step == last_step_seen) return
    last_step_seen = step
  end if

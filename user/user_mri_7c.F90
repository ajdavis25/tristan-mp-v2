...
contains
  ...
  subroutine userReadInput()
    ...
    call getInput('problem', 'backgr_T_outer',        backgr_T_outer,        4.0)
  ...
  subroutine validate_zone_geometry()
    ...
    if (inner_xmin_frac <= 0.0) then
      print *, "ERROR [7C]: inner_xmin_frac must be > 0"
      stop
    end if
    if (inner_xmax_frac >= 1.0) then
      print *, "ERROR [7C]: inner_xmax_frac must be < 1"
      stop
    end if
    if (inner_xmin_frac >= inner_xmax_frac) then
      print *, "ERROR [7C]: inner_xmin_frac must be < inner_xmax_frac"
      stop
    end if
    if (transition_width_frac <= 0.0) then
      print *, "ERROR [7C]: transition_width_frac must be > 0"
      stop
    end if
    if ((inner_xmin_frac - transition_width_frac) < 0.0) then
      print *, "ERROR [7C]: left transition extends outside the box"
      stop
    end if
    if ((inner_xmax_frac + transition_width_frac) > 1.0) then
      print *, "ERROR [7C]: right transition extends outside the box"
      stop
    end if
  ...
  subroutine load_transition_slabs(x_left, x_right, temp_left, temp_right, label)
    ...
    do i = 1, N_TRANSITION_SLABS
      ...
      print *, ">>> [7C] ", trim(label), " slab", i, " x=[", x_lo, ",", x_hi, "] T=", slab_temp
  ...
  subroutine userInitParticles()
    ...
    print *, ">>> [7C] userInitParticles: loading two-zone thermal box"
    print *, ">>> [7C] outer-left core x=[0,", zone_x0, "] T=", backgr_T_outer
    ...
    print *, ">>> [7C] inner core x=[", zone_x1, ",", zone_x2, "] T=", backgr_T_inner
    ...
    print *, ">>> [7C] outer-right core x=[", zone_x3, ",", sx_glob, "] T=", backgr_T_outer
    call load_temperature_slab(zone_x3, sx_glob, backgr_T_outer)

    if (use_rotation .and. (q_shear /= 0.0)) then
      print *, ">>> [7C] imprinting shearing flow"
      call imprint_velocity_shear()
    else
      print *, ">>> [7C] rotation/shear disabled"
    end if

    if (enable_perturbation .and. (eps_pert /= 0.0)) then
      print *, ">>> [7C] imprinting seeded perturbation"
      call imprint_perturbation()
    else
      print *, ">>> [7C] no perturbation applied"
    end if
  ...
  subroutine imprint_velocity_shear()
    ...
    print *, ">>> [7C] imprint_velocity_shear..."
    ...
    print *, ">>> [7C] imprint_velocity_shear DONE"
  ...
  subroutine imprint_perturbation()
    ...
    print *, ">>> [7C] imprint_perturbation..."

    if (abs(B0_ext) > 0.0) then
      kz = 2.0 * PI_F / real(global_mesh%sz)
      perturb_com = .false.
      print *, ">>> [7C] perturbation: localized k=1 sin/cos seed"
    else
      kz = 0.0
      perturb_com = .true.
      print *, ">>> [7C] perturbation: COM dvx kick"
    end if
    ...
    print *, ">>> [7C] imprint_perturbation DONE"
  ...
  subroutine userInitFields()
    ...
    jz(:, :, :) = 0.0
    if (mpi_rank .eq. 0) then
      print *, ">>> [7C] userInitFields initialized uniform Bz =", B0_ext
    end if

#!/usr/bin/env python3
"""
Phase 8A offline diagnostics from standard field dumps.

Inputs:
  python validate_phase8a.py <outdir> --input input_phase8a_control \
    [--save phase8a_control.png] [--csv phase8a_control_summary.csv]
"""
...
def build_config(input_path):
    ...
    inner_xmin = input_float(values, "inner_xmin_frac", 0.40)
    inner_xmax = input_float(values, "inner_xmax_frac", 0.60)
    transition = input_float(values, "transition_width_frac", 0.05)
    buffer_width = input_float(values, "buffer_width_frac", 0.125)
    ...
    if buffer_width <= 0.0:
        raise RuntimeError("buffer_width_frac must be > 0")
    zone_x0 = inner_xmin - transition
    zone_x3 = inner_xmax + transition
    if zone_x0 < 0.0 or zone_x3 > 1.0:
        raise RuntimeError("Transition region extends outside the box")
    if buffer_width >= zone_x0 or (1.0 - buffer_width) <= zone_x3:
        raise RuntimeError("Buffer overlaps the source-transition region")
    return {
        "inner_xmin_frac": inner_xmin,
        "inner_xmax_frac": inner_xmax,
        "transition_width_frac": transition,
        "buffer_width_frac": buffer_width,
        "zone_x0_frac": zone_x0,
        "zone_x1_frac": inner_xmin,
        "zone_x2_frac": inner_xmax,
        "zone_x3_frac": zone_x3,
        "left_buffer_frac": buffer_width,
        "right_buffer_frac": 1.0 - buffer_width,
    }
...
def summarize_run(data, cfg):
    ...
    left_outer_mask = region_mask(x_frac, cfg["left_buffer_frac"], cfg["zone_x0_frac"], include_right=False)
    right_outer_mask = region_mask(x_frac, cfg["zone_x3_frac"], cfg["right_buffer_frac"], include_right=False)
...
def add_zone_lines(ax, cfg):
    for xpos in (
        cfg["left_buffer_frac"],
        cfg["zone_x0_frac"],
        cfg["zone_x1_frac"],
        cfg["zone_x2_frac"],
        cfg["zone_x3_frac"],
        cfg["right_buffer_frac"],
    ):
...
def plot_results(data, cfg, summary, series, savepath):
    ...
    ax_ts.plot(times, series["e_inner"], label="source core")
    ax_ts.plot(times, series["e_outer_left"], label="left passive wing")
    ax_ts.plot(times, series["e_outer_right"], label="right passive wing")
    ...
    ax_flux.set_title(r"source-interface $S_x(t)$ (outward convention)")
    ax_flux.set_xlabel("time")
    ax_flux.set_ylabel(r"$S_x$")
    ax_flux.legend(fontsize=9)
    ax_flux.grid(True, alpha=0.3)

    fig.suptitle("phase 8A: source-plus-buffer control")
    fig.tight_layout(rect=[0, 0.03, 1, 0.96])
    fig.text(
        0.5,
        0.01,
        (
            "Zone lines mark left buffer / transition / source / transition / right buffer "
            f"at x/Lx = {cfg['left_buffer_frac']:.3f}, {cfg['zone_x0_frac']:.3f}, "
            f"{cfg['zone_x1_frac']:.3f}, {cfg['zone_x2_frac']:.3f}, "
            f"{cfg['zone_x3_frac']:.3f}, {cfg['right_buffer_frac']:.3f}."
        ),
        ha="center",
        fontsize=9,
    )
...
def build_parser():
    parser = argparse.ArgumentParser(description="Offline diagnostics for the Phase 8A source-plus-buffer control run.")
    parser.add_argument("outdir", help="Path to the Phase 8A output directory")
